...
#define DRIVER_NAME "i2c_temp"
#define DRIVER_CLASS "i2c_temp_class"

static struct i2c_adapter * i2c_temp_adapter = NULL;
static struct i2c_client * i2c_temp_client = NULL;
...
#define SLAVE_ADDRESS 0x76		/* BMP280 I2C address */  //should be verified -> done
...
static struct i2c_board_info i2c_temp_board = {
...
s32 read_temperature(void) {
	...
	/* Read Temperature */
	d1 = i2c_smbus_read_byte_data(i2c_temp_client, 0xFA);
	d2 = i2c_smbus_read_byte_data(i2c_temp_client, 0xFB);
	d3 = i2c_smbus_read_byte_data(i2c_temp_client, 0xFC);
...
static int __init ModuleInit(void) {
	...
	i2c_temp_adapter = i2c_get_adapter(I2C_BUS_AVAILABLE);

	if(i2c_temp_adapter != NULL) {
		i2c_temp_client = i2c_new_client_device(i2c_temp_adapter, &i2c_temp_board);
		if(i2c_temp_client != NULL) {
			if(i2c_add_driver(&i2c_temp_driver) != -1) {
				ret = 0;
			}
			else
				printk("Can't add driver...\n");
		}
		i2c_put_adapter(i2c_temp_adapter);
	}
	printk("BMP280 Driver added!\n");

	/* Read Chip ID */
	id = i2c_smbus_read_byte_data(i2c_temp_client, 0xD0);
	printk("ID: 0x%x\n", id);

	/* Read Calibration Values */
	dig_T1 = i2c_smbus_read_word_data(i2c_temp_client, 0x88);
	dig_T2 = i2c_smbus_read_word_data(i2c_temp_client, 0x8a);
	dig_T3 = i2c_smbus_read_word_data(i2c_temp_client, 0x8c);
	...
	i2c_smbus_write_byte_data(i2c_temp_client, 0xf5, 5<<5);
	i2c_smbus_write_byte_data(i2c_temp_client, 0xf4, ((5<<5) | (5<<2) | (3<<0)));
	return ret;
...
static void __exit ModuleExit(void) {
	...
	i2c_unregister_device(i2c_temp_client);
	i2c_del_driver(&i2c_temp_driver);
	cdev_del(&myDevice);
    device_destroy(myClass, myDeviceNr);
    class_destroy(myClass);
